...
describe('GetStatisticsUseCase Tests', () => {
    it('should return statistics for transactions within the last minute', async () => {
        ...
        // Create transactions
        await createUseCase.execute({ amount: 100, timestamp: new Date(Date.now() - 30000) }); // 30 seconds ago
        await createUseCase.execute({ amount: 200, timestamp: new Date(Date.now() - 40000) }); // 40 seconds ago
        await createUseCase.execute({ amount: 300, timestamp: new Date(Date.now() - 70000) }); // 70 seconds ago (should not be included)
